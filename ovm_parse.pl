...
foreach my $server ( $VMs->findnodes('//OvmReport/Vm') ) {
	...
	#my $state = "";
	$MAXrowid++;
	my $sql = qq(INSERT INTO vmlist (rowid,uuid,name,state,os,ram,cpus,net,cluster,owner,backup,nat) VALUES ($MAXrowid,\'$uuid\',\'$name\',\'$state\',\'$os\',\'$ram\',\'$cpus\',\'$net\',\'$cluster\',\'$owner\',\'$backup\',\'$nat\')\;);
